...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    debugLog(`[${requestId}] Open Orders Request:`, {
      timestamp: new Date().toISOString(),
      address,
    });

    // Forward the request to the actual API endpoint
    const endpoint = `/api/openOrders?address=${encodeURIComponent(address)}`;
    ...
    // Log response
    debugLog(`[${requestId}] Open Orders Response:`, {
      timestamp: new Date().toISOString(),
    ...
    debugError('Open Orders proxy error:', error);
